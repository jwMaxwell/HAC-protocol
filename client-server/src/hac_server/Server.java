...
public class Server {
    ...
    try 
    {
      ...
      Thread thread = new Thread(new SendPings());
      thread.start();
      System.out.println("2");
      Thread thread2 = new Thread(new CheckPings());
      thread2.start();
      ...
      while (true) 
      {
        ...
        if(command.equals(HACPacket.PacketType.PING))
          continue;
        
        
        if(command.equals(HACPacket.PacketType.INIT)) {
          packet_format.Node[] node = {new packet_format.Node(0, IPAddress.toString(), port)};
          HACPacket reply = new HACPacket(0,  
              (Inet4Address) Inet4Address.getLocalHost(), node);
    ...
    catch (MaxFieldCountExceededException e) {
	 ...
	 */
	private class SendPings implements Runnable{
	  ...
	  public void run() {
	    ...
	    while(true) { // while go brrr
          ...
          for(Node n : nodes) // for each node in the node list
            socket.send(new HACPacket(0,  
                (Inet4Address) n.getIp().getLocalHost(), 
	...
	private class CheckPings implements Runnable {
	  private static final long MAX_TIME = 30000;
